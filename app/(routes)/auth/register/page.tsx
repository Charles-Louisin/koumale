...
export default function RegisterPage() {
  ...
  const emailError = useMemo(() => {
    ...
    return valid ? "" : "Format d'email invalide: exemple@email.com";
  }, [formData.email, touched.email]);

  const firstNameError = useMemo(() => {
    if (!touched.firstName) return "";
    return formData.firstName.trim() ? "" : "Prénom requis";
  }, [formData.firstName, touched.firstName]);

  const lastNameError = useMemo(() => {
    if (!touched.lastName) return "";
    return formData.lastName.trim() ? "" : "Nom requis";
  }, [formData.lastName, touched.lastName]);

  const passwordError = useMemo(() => {
    if (!touched.password) return "";
    if (!formData.password) return "Mot de passe requis";
    return formData.password.length < 8 ? "Au moins 8 caractères" : "";
  }, [formData.password, touched.password]);

  const confirmPasswordError = useMemo(() => {
    if (!touched.confirmPassword) return "";
    return formData.password === formData.confirmPassword ? "" : "Les mots de passe ne correspondent pas";
